...
object MyAnimeListClient:
  val noop: MyAnimeListClient = new:
    def searchManga(token: AuthToken, term: Term.Name) =
      IO.pure(GetMangaListSuccess(Nil))
    def find(token: AuthToken, mangaId: ExternalMangaId) = IO.pure(None)
    def updateStatus(
        token: AuthToken,
        mangaId: ExternalMangaId,
        latestChapter: LatestChapter
    ) = IO.unit
    def refreshAuthToken(token: RefreshToken) =
      IO.pure(AuthToken(0L, RefreshToken("noop"), AccessToken("noop")))
    def acquireToken(code: String, codeChallenge: String) =
      IO.pure(AuthToken(0L, RefreshToken("noop"), AccessToken("noop")))
    def generateCodeChallenge = IO.pure("noop-challenge")
    def createAuthorizationLink(codeChallenge: String) = uri"https://noop"
