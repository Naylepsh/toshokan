...
object AssetRepository:
  case class Column[A: Read: Put](name: String):
  ...
  object Columns:
    ...
    def apply[A1, A2](t: (Column[A1], Column[A2])): Columns[(A1, A2)] =
      ...
      val a: Fragment = t._1
      val b: Fragment = t._2
      new Columns(concatFrags(List(a, b)))


    // def apply[A1, A2](t: (Column[A1], Column[A2])): Columns[(A1, A2)] =
    //   new Columns(
    //     concatFrags(
    //       Iterable.from(t.productIterator.map(_.asInstanceOf[Fragment]))
    //     )
    //   )
    //
  ...
  object TableDefinition:
    ...
    given Conversion[TableDefinition, SingleFragment[Nothing]] = table => fr"${table.name}"
